...
class Track:
    ...
    last_seen_frame: int
    last_bbox: Tuple[int, int, int, int]          # xyxy px
    prev_center: Tuple[float, float]              # cx, cy px
    prev_h: float                                 # bbox height px
...
class MultiPersonTracker:
    ...
    - matches detections to tracks with IoU + center distance + height similarity
    - keeps a separate JumpDetector+StateMachine per track_id
    """
    ...
    def __init__(
        ...
        height_gate: float = 0.40,  # ±40% height
    ):
        self.fps = float(fps)
    ...
    def _match_score(self, tr: Track, det57: np.ndarray, w: int, h: int) -> Optional[float]:
        ...
        # IoU gate (more stable when people overlap/cross)
        iou = iou_xyxy(tr.last_bbox, bb)
        if iou < self.iou_gate:
            return None

        # score: prefer larger IoU, smaller distance (normalized)
    ...
    def associate(self, frame_idx: int, dets: np.ndarray, w: int, h: int) -> Dict[int, int]:
        """
        Returns mapping {track_id: det_index}. Greedy best-first assignment.
        Also spawns new tracks for unmatched detections and prunes stale tracks.
        """
        if dets.shape[0] == 0:
    ...
    def _prune(self, frame_idx: int):
        ...
        for tid, tr in self.tracks.items():
            if (frame_idx - tr.last_seen_frame) > self.max_missing_frames:
                dead.append(tid)
        for tid in dead:
            del self.tracks[tid]
...
def main():
    ...
    # Multi-person tracker (each person has their own detector + count)
    ...
    while True:
        ...
        if valid.shape[0] == 0:
            # still write frame (optional); keeping behavior close to your old script:
            writer.write(frame)
            frame_idx += 1
            continue

        # Associate detections to persistent IDs
        ...
        for tid, det_idx in assign.items():
            ...
            # Draw pose (bbox uses per-track color)
            ...
            # Per-person HUD label near bbox
            x1, y1, x2, y2 = bb
            ...
            # Save per-person row (multiple rows per frame)
            frame_data.append({
                "frame_idx": frame_idx,
                "timestamp": t_sec,
                "track_id": tid,
    ...
    # Print final counts per person
    print("DONE")
    if len(mp.tracks) == 0:
        print("No active tracks at end.")
    else:
        print("Active track counts at end:")
        for tid, tr in sorted(mp.tracks.items(), key=lambda kv: kv[0]):
            print(f"  ID {tid}: {tr.detector.jump_count}")
