...
VIDEO_SOURCE = "TestVideos/108.2.mp4"
IMG_SIZE = 640
...
def det_to_xy(det57: np.ndarray, w: int, h: int) -> np.ndarray:
    kpts = decode_kpts_17x3(det57)
    return map_norm_to_frame_xy(kpts[:, :2], w, h)
...
def draw_pose_from_xy(frame: np.ndarray, xy: np.ndarray):
    pts = []
    h, w = frame.shape[:2]
...
def get_kpt_xy_from_xy(xy: np.ndarray, idx: int) -> Optional[Tuple[float, float]]:
    return float(xy[idx, 0]), float(xy[idx, 1])


def build_kpt_csv_columns():
    cols = ["frame_idx", "timestamp", "conf", "person_h"]
    for i in range(17):
...
def xy_to_kpt_csv_row(frame_idx: int, timestamp: float, conf: float, person_h: float, xy: np.ndarray):
    row = [frame_idx, timestamp, conf, person_h]
    for i in range(17):
...
def smooth_xy_sequence(xy_seq, preferred_wl: int, polyorder: int) -> np.ndarray:
    """
    xy_seq: list of N arrays, shape (17,2)
    returns: smoothed array shape (N,17,2)
    """
...
class JumpDetector:
    """
    - Airborne detection & landing event: HIP-based
    - Cycle/cadence vote: SHOULDER-driven
    - Amplitude gate: SHOULDER amplitude
    - Dynamic lift threshold: HIP amplitude EWMA + ground noise sigma
    """
...
def main():
    ...
    out_video_path = f"jump_rope_results/{base}_cleaned_jump.mp4"
    out_csv_path = f"jump_rope_results/{base}_jump_metrics.csv"
    raw_kpt_csv_path = f"jump_rope_results/{base}_raw_kpts.csv"
    cleaned_kpt_csv_path = f"jump_rope_results/{base}_cleaned_kpts.csv"
    ...
    # rolling buffers for streaming clean+count
    frame_buffer = deque()
    meta_buffer = deque()   # (frame_idx, t_sec, conf, person_h, infer_ms)
    xy_buffer = deque()

    with open(raw_kpt_csv_path, "w", newline="", encoding="utf-8-sig") as f_raw, \
         open(cleaned_kpt_csv_path, "w", newline="", encoding="utf-8-sig") as f_clean:

        raw_csv_writer = csv.writer(f_raw)
        cleaned_csv_writer = csv.writer(f_clean)

        raw_csv_writer.writerow(build_kpt_csv_columns())
        cleaned_csv_writer.writerow(build_kpt_csv_columns())
        ...
        while True:
            ...
            if valid.shape[0] == 0:
                ...
                best_conf = 0.0
                person_h = float(prev_h_px) if prev_h_px is not None else 0.0
                xy = prev_xy.copy()
            else:
                if prev_center_px is None:
                    ...
                    centers_px = np.array(
                        [bbox_center_px(d, width, height) for d in valid],
                        dtype=np.float32
                    )
                    dist2 = (
                        (centers_px[:, 0] - float(prev_center_px[0])) ** 2
                        + (centers_px[:, 1] - float(prev_center_px[1])) ** 2
                    )

                    heights_px = np.array(
                        [bbox_height_px(d, width, height) for d in valid],
                        dtype=np.float32
                    )
                ...
                person_h = float(prev_h_px)
                xy = det_to_xy(best, width, height)
                prev_xy = xy.copy()

            # -------- write raw keypoint csv immediately --------
            raw_csv_writer.writerow(
                xy_to_kpt_csv_row(frame_idx, t_sec, best_conf, person_h, xy)
            )

            # -------- push into rolling buffers --------
            frame_buffer.append(frame.copy())
            meta_buffer.append((frame_idx, t_sec, best_conf, person_h, infer_ms))
            xy_buffer.append(xy.copy())

            # -------- once enough frames, emit oldest cleaned frame --------
            if len(xy_buffer) >= SG_WINDOW_LENGTH:
                ...
                out_frame_idx, out_t_sec, out_conf, out_person_h, out_infer_ms = meta_buffer[0]

                # cleaned keypoint CSV
                cleaned_csv_writer.writerow(
                    xy_to_kpt_csv_row(out_frame_idx, out_t_sec, out_conf, out_person_h, out_xy)
                )

                # cleaned keypoints -> jump algorithm
                lhip = get_kpt_xy_from_xy(out_xy, LEFT_HIP)
                rhip = get_kpt_xy_from_xy(out_xy, RIGHT_HIP)
                lank = get_kpt_xy_from_xy(out_xy, LEFT_ANKLE)
                rank = get_kpt_xy_from_xy(out_xy, RIGHT_ANKLE)
                lsho = get_kpt_xy_from_xy(out_xy, LEFT_SHOULDER)
                rsho = get_kpt_xy_from_xy(out_xy, RIGHT_SHOULDER)

                det = detector.update(
                    out_frame_idx, out_t_sec,
                    lhip, rhip, lank, rank, lsho, rsho,
                    out_person_h
                )
                det["jump_count"] = sm.update(out_t_sec, det["jump_count"])
                detector.jump_count = det["jump_count"]

                draw_pose_from_xy(out_frame, out_xy)
                draw_hud(out_frame, det, sm.state, out_infer_ms)
                ...
                cv2.imshow("Cleaned Jump Pipeline", out_frame)
    ...
    print("Jump metrics csv:", out_csv_path)
    print("Raw kpt csv:", raw_kpt_csv_path)
    print("Cleaned kpt csv:", cleaned_kpt_csv_path)
